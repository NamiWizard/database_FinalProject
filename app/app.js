...
function WeekAllAccount(fn) {
    client.getSession().then(session => {
        //讓傳進來的DATA符合一般日期:DATE_FORMAT(`order_date`, '%Y-%m-%d')
        session.sql("SELECT SUM(order_price),DATE_FORMAT(`order_date`, '%V')FROM `order_record`  Group by DATE_FORMAT(`order_date`,'%m'  )ORDER BY DATE_FORMAT(`order_date`,'%m'  )").execute().then(function (result) {
            fn(result.fetchAll());
        }).catch(console.error);
        return session.close();
    }).catch(console.error);
...
function MonthAllAccount(fn) {
    client.getSession().then(session => {
        //讓傳進來的DATA符合一般日期:DATE_FORMAT(`order_date`, '%Y-%m-%d')
        session.sql("SELECT SUM(order_price),DATE_FORMAT(`order_date`, '%m')FROM `order_record`  Group by DATE_FORMAT(`order_date`,'%m'  )ORDER BY DATE_FORMAT(`order_date`,'%m'  )").execute().then(function (result) {
            fn(result.fetchAll());
        }).catch(console.error);
        return session.close();
    }).catch(console.error);
...
app.get('/userClient', function (req, res) {
    RetriveAllClient(function (all_result) {
        CountClientStatus(function (status_count) {
            WeekAllAccount(function (week_account) {
                MonthAllAccount(function (month_account) {

                    res.render('user_client', {
                        'user_client_search': [],
                        'user_client_data': all_result,
                        'user_client_status': status_count,
                        'user_company_week_account': week_account,
                        'user_company_month_account': month_account,
                        'update': false
                    });
                });
            });
